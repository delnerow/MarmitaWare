...
def test_app_integracao_create_e_load_ingrediente(db_test_file):
    ...
    2. Verifica se o item foi salvo no BD
    3. Verifica se o item foi salvo na memória
    4. Verifica se o LoadIngredientes (no __init__ de um novo App)
       consegue carregar o item salvo.
    CORRIGIDO: O CreateIngrediente agora salva na memória também
    ...
    # CORRIGIDO: O método CreateIngrediente agora salva na memória também
    assert len(app.ingredientes) == 1 # O CreateIngrediente salvou na memória
    assert 1 in app.ingredientes # O ID é 1
    assert app.ingredientes[1].nome == "Tomate"
