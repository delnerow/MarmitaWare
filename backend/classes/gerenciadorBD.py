...
class GerenciadorBD():
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.data_root = os.path.join(cls._instance.current_path, '..', 'database')
            cls._instance.DATA_FILE = os.path.join(cls._instance.data_root, 'dados.db')
    ...
    def GetComprasTable(self):
        """Retorna um pandas.DataFrame com colunas: 'ingredientes comprados' / 'valor total' / 'data'."""
        conn = sqlite3.connect(self.DATA_FILE)
        try:
            query = """
                SELECT c.id_compra,
    ...
    def GetMarmitasTable(self):
        """Retorna um pandas.DataFrame com colunas: 'nome_marmita' / 'preco_venda' / 'custo_estimado' / 'ingredientes'"""
        conn = sqlite3.connect(self.DATA_FILE)
        try:
            query = """
                SELECT m.id_marmita,
    ...
    def getVendasTable(self):
        """Retorna um pandas.DataFrame com colunas: 'data' / 'nome_marmita' / 'quantidade_vendida' / 'preco_venda' / 'valor_total'"""
        conn = sqlite3.connect(self.DATA_FILE)
        try:
            query = """
                SELECT v.id_venda,
    ...
    def updateIngrediente(self, ingrediente):
        """Atualiza um ingrediente existente no banco"""
        conn = sqlite3.connect(self.DATA_FILE)
        cursor = conn.cursor()
        ...
        try:
            cursor.execute('''
                UPDATE Ingredientes
                SET nome_ingrediente = ?,
                    preco_compra = ?,
                    data_ultima_compra = ?,
                    id_unidade = ?
                WHERE id_ingrediente = ?
            ''', (ingrediente.nome, ingrediente.preco_compra, 
                  ingrediente.data_ultima_compra, ingrediente.id_unidade,
                  ingrediente.ID))
            conn.commit()
        except sqlite3.IntegrityError as e:
            conn.rollback()
    ...
    def updateMarmita(self, marmita):
        """Atualiza uma marmita existente no banco"""
        conn = sqlite3.connect(self.DATA_FILE)
        cursor = conn.cursor()
        ...
        try:
            cursor.execute('''
                ...
                SET nome_marmita = ?,
                    ...
                    custo_estimado = ?
                WHERE id_marmita = ?
            ''', (marmita.nome, marmita.preco_venda, 
                  marmita.custo_estimado, marmita.ID))
            
            cursor.execute('DELETE FROM ingredientes_marmita WHERE id_marmita = ?', 
                          (marmita.ID,))
            
            if hasattr(marmita, 'quantidade_ingredientes') and marmita.quantidade_ingredientes:
                for id_ingrediente, quantidade in marmita.quantidade_ingredientes.items():
                    cursor.execute('''
                        INSERT INTO ingredientes_marmita (id_marmita, id_ingrediente, quantidade)
                        VALUES (?, ?, ?)
                    ''', (marmita.ID, id_ingrediente, quantidade))
    ...
    def updateVenda(self, venda):
        """Atualiza uma venda existente no banco"""
        conn = sqlite3.connect(self.DATA_FILE)
        cursor = conn.cursor()
        ...
        try:
            cursor.execute('''
                ...
                SET id_marmita = ?,
                    ...
                    quantidade_vendida = ?
                WHERE id_venda = ?
            ''', (venda.id_marmita, venda.data_de_venda, 
                  venda.quantidade_vendida, venda.ID))
            conn.commit()
        except sqlite3.IntegrityError as e:
            conn.rollback()
            raise e
    ...
    def updateCompra(self, compra):
        """Atualiza uma compra existente no banco"""
        conn = sqlite3.connect(self.DATA_FILE)
        cursor = conn.cursor()
        ...
        try:
            cursor.execute('''
                ...
                SET data_de_compra = ?,
                    valor_total = ?
                WHERE id_compra = ?
            ''', (compra.data, compra.valor_total, compra.ID))
            
            cursor.execute('DELETE FROM compra_ingredientes WHERE id_compra = ?', 
                          (compra.ID,))
            
            if hasattr(compra, 'preco_ingredientes') and compra.preco_ingredientes:
                for id_ingrediente, preco in compra.preco_ingredientes.items():
                    cursor.execute('''
                        INSERT INTO compra_ingredientes (id_compra, id_ingrediente, preco_compra)
                        VALUES (?, ?, ?)
                    ''', (compra.ID, id_ingrediente, preco))
                    
                    cursor.execute('''
                        UPDATE Ingredientes
                        SET preco_compra = ?, data_ultima_compra = ?
                        WHERE id_ingrediente = ?
                    ''', (preco, compra.data, id_ingrediente))
    ...
    def deleteIngrediente(self, id_ingrediente: int):
        """Remove um ingrediente do banco"""
        conn = sqlite3.connect(self.DATA_FILE)
        cursor = conn.cursor()
        
        try:
            cursor.execute('DELETE FROM Ingredientes WHERE id_ingrediente = ?', 
                          (id_ingrediente,))
            conn.commit()
        except sqlite3.IntegrityError as e:
            conn.rollback()
            raise ValueError("Não é possível remover: ingrediente está sendo usado em marmitas ou compras")
        finally:
            conn.close()

    def deleteMarmita(self, id_marmita: int):
        """Remove uma marmita do banco"""
        conn = sqlite3.connect(self.DATA_FILE)
        cursor = conn.cursor()
        ...
        try:
            ...
            cursor.execute('DELETE FROM Marmitas WHERE id_marmita = ?', 
                          (id_marmita,))
            conn.commit()
        except sqlite3.IntegrityError as e:
            conn.rollback()
            raise ValueError("Não é possível remover: existem vendas registradas desta marmita")
        finally:
            conn.close()

    def deleteVenda(self, id_venda: int):
        """Remove uma venda do banco"""
        conn = sqlite3.connect(self.DATA_FILE)
        cursor = conn.cursor()
        
        try:
            cursor.execute('DELETE FROM Vendas WHERE id_venda = ?', 
                          (id_venda,))
            conn.commit()
        finally:
            conn.close()
    ...
    def deleteCompra(self, id_compra: int):
        ...
        try:
            ...
            cursor.execute('DELETE FROM Compras WHERE id_compra = ?', 
                          (id_compra,))
            conn.commit()
        finally:
            conn.close()