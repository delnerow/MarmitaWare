...
// URL base da API - para Android emulador use 10.0.2.2, para dispositivo físico use o IP da máquina
// Você pode alterar isso no app.json em expo.extra.apiUrl
const getApiUrl = () => {
  if (__DEV__) {
    // Em desenvolvimento, tenta usar a URL configurada ou padrão
    return Constants.expoConfig?.extra?.apiUrl || 'http://10.73.1.219:5000/api';
  }
  // Em produção, você pode configurar uma URL diferente
  return Constants.expoConfig?.extra?.apiUrl || 'http://10.73.1.219:5000/api';
};
...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response) {
      ...
      throw new Error('Servidor não está respondendo. Verifique se a API está rodando e se o IP está correto.');
    } else {
